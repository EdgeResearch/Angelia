import requests_html
import pyppeteer.errors
from bs4 import BeautifulSoup
from tqdm import tqdm
import pandas as pd

session = requests_html.HTMLSession()
df = pd.read_csv('../datasets/dataset_2.csv', sep=';')
data = {
    'url': [],
    'title_result': []
}
results_df = pd.DataFrame(data)


def extract_title(site_url):
    response = session.get(site_url)
    try:
        response.html.render(timeout=20)
    except pyppeteer.errors.PageError:
        return " "
    except requests_html.MaxRetries:
        return " "
    soup = BeautifulSoup(response.html.html, 'html.parser')
    headlines = []
    for headline in soup.find_all('title'):
        headlines.append(headline.text)
    if len(headlines) > 0:
        return max(headlines, key=len)
    return " "


try:
    with tqdm(total=len(df)) as pbar:
        for row in df.itertuples(index=True):
            extracted_title = extract_title(row.url)
            new_row = {'url': row.url, 'title_result': extracted_title}
            results_df.loc[len(results_df)] = new_row
            pbar.update(1)
finally:
    pbar.close()
    results_df.to_csv('./results/title_extracted.csv', sep=';', index=False)
